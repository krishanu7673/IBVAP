...
from src.contextual_threat import (
    ContextualThreatEngine
)
...
class BorderAnalyticsEngine:
    ...
    def __init__(
        ...
        # ==========================================================
        # CONTEXT-AWARE MULTI-EVENT THREAT CORRELATION
        # ==========================================================

        self.contextual_threat = ContextualThreatEngine(
            correlation_window=60.0,
            cooldown_seconds=20.0
        )

        print(
            "[*] Context-aware multi-event threat "
            "correlation enabled."
        )
    ...
    def _extract_appearance_feature(
        ...
        for cross-camera entity correlation.
        ...
        The descriptor consists of:
            ...
            32-bin Saturation histogram
            32-bin Value histogram
            32-bin Grayscale histogram

        Total feature dimension = 128.

        The feature is L2-normalized so that cosine similarity
        can be used by the cross-camera entity registry.
        """
    ...
    def _cleanup_tracks(
        ...
        # ----------------------------------------------------------
        # Contextual threat cleanup
        # ----------------------------------------------------------

        self.contextual_threat.cleanup(
    ...
    # ==============================================================
    # CONTEXTUAL THREAT PROCESSING
    # ==============================================================

    def _process_contextual_threat(
        self,
        camera_id,
        track_id,
        behavior,
        ...
        """
        Feed significant events belonging to the current track
        into the contextual threat engine.

        The contextual engine looks for combinations such as:

        EARLY_WARNING + ZONE_ENTRY
        EARLY_WARNING + BEHAVIOR_RISK
        ZONE_ENTRY + LOITERING
        LOITERING + BEHAVIOR_RISK
        PERIMETER_BREACH + other significant activity

        It does not attempt to determine criminal intent.
        It correlates observable events within a time window.
        """
        ...
        try:
            ...
            for existing_event in events:
                ...
                # Do not feed contextual alerts back into
                # the contextual engine itself.
            ...
            if not current_track_events:
                return None

            contextual_result = None

            # Record each significant event generated during
            # this frame for the current track.

            for existing_event in current_track_events:
                ...
                contextual_result = (
                    self.contextual_threat.record_event(
                        camera_id=camera_id,
                        track_id=track_id,
                        event_type=event_type,
                        risk_score=existing_event.get(
                            "risk_score",
                            behavior.get(
                                "risk_score",
                                0
                ...
                # The contextual engine may return an alert
                # only when sufficient correlated evidence exists.
                ...
                if contextual_result is not None:
                    ...
                    return self._build_event(
                        camera_id=camera_id,
                        event_type="CONTEXTUAL_THREAT",
                        details=(
                            f"Track ID {track_id} generated "
                            f"a correlated multi-event threat "
                            f"pattern with contextual risk "
                            f"{contextual_result.get('contextual_score', 0)}/100"
                        ),
                        track_id=track_id,
                        confidence=None,
                        behavior=contextual_behavior,
                        extra={
                            ...
                            "contextual_event_count":
                                contextual_result.get(
                                    "event_count",
                                    0
                                ),

                            "contextual_event_types":
                                contextual_result.get(
                                    "event_types",
                                    []
                                ),
        ...
        for index in range(
            ...
            if track_id >= 0:
                ...
                try:
                    ...
                    spatial_result = (
                        self.spatial_intelligence.analyze(
                            track_id=track_id,
                            center=center
                        )
                    )
            ...
            if label == self.PERSON_CLASS:
                ...
                if track_id >= 0:
                    ...
                    # ------------------------------------------------
                    # CONTEXT-AWARE MULTI-EVENT CORRELATION
                    # ------------------------------------------------

                    contextual_event = (
                        self._process_contextual_threat(
                            camera_id=camera_id,
                            track_id=track_id,
                            behavior=behavior,
                            events=events
            ...
            detections.append({
                ...
                "appearance_feature_available":
                    appearance_feature is not None
            })
